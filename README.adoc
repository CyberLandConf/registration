...
=== Development with profile `local`
...
The container will start and initialize all the needed resoures like PostgreSQL and Mailpit.
* (Optional) Build application: `mvn clean package`
* Run application with `local` profile:
** With Maven, use https://quarkus.io/guides/maven-tooling#development-mode[Quarkus Development Mode]: `mvn compile quarkus:dev -Dquarkus.profile=local`
+
...
+
IMPORTANT: Change the `dev` profile in the image with `local`
+
